...
/**
 * @ngdoc function
 * @name Elidom.Base.controllers:ServiceDetailCtrl
 * @description
 * # Service Detail Controller
 */
angular.module('Elidom.Base')
	.controller('ServiceDetailCtrl', function($rootScope, $scope, $state, $stateParams, API_ENDPOINT, MenuService, RestApiService) {
		 ...
		 * 서비스 상세 조회 URL
		 */
		var serviceUrl = '/core/service/api/findService.json';
		/**
		 * 실행 중 여부 - For Spinner
		 */
		...
		$scope.goApiDetail = function(item) {
			$state.go('app.service-api-detail', { id : item.id });
		};
